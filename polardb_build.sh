...
BLOCK_SIZE=32        # 数据块大小 (1-32)
WAL_BLOCK_SIZE=16    # WAL块大小 (例如：8、16，单位KB)
...
if [[ $nocompile == "off" ]];
then
  ./configure --prefix=$pg_bld_basedir --with-pgport=$pg_bld_port $common_configure_flag $configure_flag --with-blocksize=$BLOCK_SIZE --with-wal-blocksize=$WAL_BLOCK_SIZE --disable-debug --disable-cassert --disable-dtrace --disable-profiling 
...
######################### pg_bulkload ###########################
# 设置 pg_bulkload 的目录
PGBULKLOAD_DIR=./pg_bulkload-3.1.20

# 检查 pg_bulkload 目录是否存在
if [ ! -d "$PGBULKLOAD_DIR" ]; then
  echo "pg_bulkload directory not found!"
  exit 1
fi

# 清理之前的构建文件
echo "Cleaning previous builds..."
make -C "$PGBULKLOAD_DIR" clean
if [ $? -ne 0 ]; then
  echo "Clean failed!"
  exit 1
fi
...
# 编译 pg_bulkload
echo "Compiling pg_bulkload..."
make -C "$PGBULKLOAD_DIR" USE_PGXS=1
if [ $? -ne 0 ]; then
...
# 安装 pg_bulkload
echo "Installing pg_bulkload..."
make -C "$PGBULKLOAD_DIR" USE_PGXS=1 install
if [ $? -ne 0 ]; then
  echo "Installation failed!"
  exit 1
fi

echo "pg_bulkload compiled and installed successfully!"

# 设置 pg_hint_plan 的目录
PGHINTPLAN_DIR=./pg_hint_plan-PG11

# 检查 pg_hint_plan 目录是否存在
if [ ! -d "$PGHINTPLAN_DIR" ]; then
  echo "pg_hint_plan directory not found!"
  exit 1
fi

# 清理之前的构建文件
echo "Cleaning previous builds..."
make -C "$PGHINTPLAN_DIR" clean
if [ $? -ne 0 ]; then
  echo "Clean failed!"
  exit 1
fi
...
# 编译 pg_hint_plan
echo "Compiling pg_hint_plan..."
make -C "$PGHINTPLAN_DIR" USE_PGXS=1
if [ $? -ne 0 ]; then
  echo "Compilation failed!"
  exit 1
fi
...
make -C "$PGHINTPLAN_DIR" USE_PGXS=1 install
if [ $? -ne 0 ]; then
  echo "Installation failed!"
  exit 1
fi

echo "pg_hint_plan compiled and installed successfully!"


######################### PHASE 7 Change ###########################
# 并行度控制
# SET max_worker_processes = 64;             -- 适当增大工作进程数
psql -c "alter system set max_worker_processes = 128;"
# SET max_parallel_workers_per_gather = 16;   -- 单个查询使用8个worker
psql -c "alter system set max_parallel_workers_per_gather = 16;" ########
psql -c "alter system set max_parallel_workers = 128;"
psql -c "alter system set max_parallel_maintenance_workers = 16;" #########

# min( 8G, (主机内存*1/8)/max_parallel_maintenance_workers )
psql -c "alter system set maintenance_work_mem = '2GB';" #64MB
...
psql -c "alter system set full_page_writes = off;"
psql -c "alter system set max_wal_size = '8GB';" # 2*shared_buffers
psql -c "alter system set min_wal_size = '2GB';" # 1/2*shared_buffers

# 如果设置了synchronous_commit = off，建议设置wal_writer_delay      
psql -c "alter system set wal_writer_delay = '10ms';"
psql -c "alter system set wal_writer_flush_after = '1MB';"

# checkpoint
psql -c "alter system set checkpoint_timeout = '30min';"
psql -c "alter system set checkpoint_completion_target = 0.99;"

psql -c "ALTER SYSTEM SET polar_enable_shm_aset = 'on';"
psql -c "ALTER SYSTEM SET polar_ss_shared_memory_size = '1048576';"
psql -c "ALTER SYSTEM SET polar_ss_dispatcher_count = 8;"
psql -c "ALTER SYSTEM SET polar_enable_shared_server = 'on';"
psql -c "ALTER SYSTEM SET polar_ss_backend_max_count = 80;"
psql -c "ALTER SYSTEM SET polar_ss_backend_idle_timeout = '3min';"
psql -c "ALTER SYSTEM SET polar_ss_session_wait_timeout = '60s';"

# max_connections 物理内存(GB)*1000*(1/4)/5 
psql -c "alter system set max_connections = 500;"
psql -c "alter system set superuser_reserved_connections =10;"
...
# 设置写 I/O 缓存策略，例如 4096
psql -c "alter system set polar_bt_write_page_buffer_size = 4096;"

# 根据指导新参数
#wal_buffers 建议 min( WAL segment size(默认16MB) , shared_buffers/32 ) ,减小
psql -c "alter system set wal_buffers = '8MB';"
# wal_compression
psql -c "alter system set wal_compression = 'on';"

#deadlock_timeout
psql -c "alter system set deadlock_timeout = '2s';"
# log_statement
psql -c "alter system set log_statement = 'none';"
# old_snapshot_threshold
psql -c "alter system set old_snapshot_threshold = -1;"

# 预分配
psql -c "ALTER SYSTEM SET polar_bulk_extend_size = '32MB';"  
psql -c "ALTER SYSTEM SET polar_index_create_bulk_extend_size = 1024;" 

# SET track_counts TO off;
psql -c "alter system set track_counts = off;"
# SET track_activities TO off;
psql -c "alter system set track_activities = off;"

# auto_explain.sample_rate = 0.01减少日志量
psql -c "alter system set auto_explain.sample_rate = 0;"

# digoal
psql -c "alter system set random_page_cost = 1.1;"

psql -c "alter system set shared_buffers = '512MB';" #初始128MB，导入版2GB，digoal 8G
...
psql -c "alter system set vacuum_cost_limit = 1500;"
psql -c "alter system set vacuum_cost_page_hit = 0;"
psql -c "alter system set vacuum_cost_page_miss = 1;"
psql -c "alter system set vacuum_cost_page_dirty = 10;"
...
# 重启后
psql -c "LOAD 'pg_hint_plan';"

# 重新挂载 /dev/shm，设置更大的大小（例如 2GB）
